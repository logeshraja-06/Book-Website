...
export default function AuthorRegisterPage() {
  ...
  const validateField = (field, values = formData) => {
    ...
    if (field === 'acceptTerms') {
      if (!values.acceptTerms) {
        err = 'You must accept the terms to apply as an author.';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsLoading(false);
      navigate('/login?registered=author', { replace: true });
    } catch (err) {
      setIsLoading(false);
      ...
      if (msg.includes('already exists') || msg.includes('409')) {
        ...
        setServerError(msg || 'Author registration failed. Please try again.');
      }
    }
  };
      ...
      <div className="w-full max-w-lg space-y-8">
        ...
        <div className="text-center space-y-3">
          <div className="w-12 h-12 rounded-2xl bg-[#F4EEEA] border border-[#E7D9D3] flex items-center justify-center text-[#2B2B2B] mx-auto">
            <Feather className="w-6 h-6 text-[#D3968C]" />
          </div>
          ...
          <h1 className="font-editorial-serif text-4xl text-[#2B2B2B] font-normal">
            Become a Studio Author
          </h1>
          <p className="text-xs text-[#6E6A67]">
            Publish your manuscripts, track analytics, and reach global readers.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <Input
                ...
                placeholder="e.g. Ramaswamy Krishnamurthy"
              />
            ...
            <Input
              ...
              placeholder="e.g. kalki@bookverse.in"
            />
            ...
            <Input
              label="Short Author Bio"
              type="textarea"
              rows={3}
              value={formData.bio}
              onChange={(e) => handleChange('bio', e.target.value)}
              placeholder="Briefly describe your writing experience and literary genre focus..."
            />
