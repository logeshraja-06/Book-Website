...
export default function FeaturedSection() {
  const spotlightBook = FEATURED_BOOKS[0]; // Ponniyin Selvan
  const secondaryBooks = FEATURED_BOOKS.slice(1, 5); // 4 supporting titles
    ...
    <section id="featured" className="py-24 bg-[#F4EEEA] border-y border-[#E7D9D3]">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          {/* Spotlight Hero Book (Entire card wrapped in Link) */}
          <motion.div 
          ...
          >
            <Link to={`/books/${spotlightBook.id}`} className="block space-y-6">
              <div className="flex items-center justify-between">
                <span className="px-3 py-1 rounded-full bg-[#E8C8C2]/40 text-[#2B2B2B] text-xs font-medium tracking-wide">
                  Editor's Spotlight
                </span>
                <span className="text-xs font-mono text-[#6E6A67]">ISBN {spotlightBook.isbn}</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-12 gap-6 items-center">
                <div className="sm:col-span-5 aspect-[3/4] rounded-xl overflow-hidden shadow-md bg-[#F4EEEA]">
                  <img 
                ...
                <div className="sm:col-span-7 space-y-3">
                  <span className="text-xs uppercase tracking-wider font-mono text-[#D3968C]">
                    {spotlightBook.genre}
                  </span>
                  <h3 className="font-editorial-serif text-2xl font-bold text-[#2B2B2B] leading-tight group-hover:text-[#C98579] transition-colors">
                    {spotlightBook.title}
                  </h3>
                  <p className="text-sm text-[#6E6A67]">By <span className="text-[#2B2B2B] font-medium">{spotlightBook.author}</span></p>
            ...
            <div className="pt-6 border-t border-[#E7D9D3] flex items-center justify-between mt-6">
              <div>
                ...
                <span className="font-editorial-serif text-2xl font-bold text-[#2B2B2B]">₹{spotlightBook.price.toLocaleString()}</span>
              </div>
              <Button to={`/books/${spotlightBook.id}`} size="sm" icon={ArrowUpRight}>
                View Details
              </Button>
            </div>
          </motion.div>
          ...
          <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {secondaryBooks.map((book, idx) => (
              ...
              >
                <Link
                  to={`/books/${book.id}`}
                  className="bg-[#FFFFFF] rounded-2xl p-5 border border-[#E7D9D3] flex flex-col justify-between hover:border-[#D3968C] transition-all duration-300 group block h-full"
                >
                  <div>
                    <div className="aspect-[4/3] rounded-lg overflow-hidden bg-[#F4EEEA] mb-4">
                      <img 
                        src={book.coverUrl} 
                        alt={book.title} 
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                  ...
                  <div className="pt-4 mt-4 border-t border-[#E7D9D3]/60 flex items-center justify-between">
                    <span className="font-editorial-serif text-base font-semibold text-[#2B2B2B]">₹{book.price}</span>
