...
export default function BookCover({
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-[9px] uppercase tracking-[0.18em] font-editorial-sans text-[#7B021D] font-bold">
            {genre}
          </span>
          <div className="w-5 h-5 rounded-md bg-[#E9E5C8]/60 flex items-center justify-center">
            <BookOpen className="w-3 h-3 text-[#7B021D]" />
          </div>
        </div>
  ...
  return (
    <div
      className={`relative ${baseAspect} rounded-xl overflow-hidden bg-[#F5F5DA] border border-[#E9E5C8]/80 ${
        showShadow ? 'shadow-xs' : ''
      } ${className}`}
    >
      {variant === '3d' && <div className="book-spine-depth" />}
      <img
        src={coverSrc}
        alt={alt || title}
        onError={() => setImageError(true)}
        className={`w-full h-full object-cover transition-transform duration-600 ease-out ${imageClassName}`}
        loading="lazy"
      />
      <div className="absolute inset-0 bg-gradient-to-t from-[#211D1D]/35 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
    </div>
  );
}
