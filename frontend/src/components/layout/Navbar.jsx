  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[#FAF8F6]/90 backdrop-blur-md border-b border-[#E7D9D3] shadow-sm py-3'
          : 'bg-[#FAF8F6] border-b border-[#E7D9D3]/60 py-4'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-[#F4EEEA] border border-[#E7D9D3] flex items-center justify-center text-[#2B2B2B] group-hover:border-[#D3968C] transition-colors duration-300 shadow-sm">
            <BookOpen className="w-5 h-5 text-[#2B2B2B] group-hover:text-[#D3968C] transition-colors duration-300" />
          </div>
          <div className="flex flex-col">
            <span className="font-editorial-serif text-xl tracking-tight font-semibold text-[#2B2B2B] flex items-center gap-1">
              BookVerse <span className="text-[#D3968C] inline-block w-1.5 h-1.5 rounded-full bg-[#D3968C]"></span>
            </span>
            <span className="text-[10px] tracking-widest uppercase font-mono text-[#6E6A67]">Studio Ecosystem</span>
          </div>
        </Link>

        {/* Desktop Centered Navigation Links */}
        <nav className="hidden lg:flex items-center gap-8 text-sm font-medium text-[#6E6A67]">
          {navLinks.map((link) => {
            const isActive =
              link.path === '/'
                ? location.pathname === '/'
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            {currentUser && (
              <div className="p-4 rounded-2xl bg-[#F4EEEA] border border-[#E7D9D3] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <img
                    src={currentUser.avatarUrl}
                    alt={currentUser.name}
            ...
            <nav className="flex flex-col gap-3 text-sm font-medium text-[#6E6A67]">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`hover:text-[#2B2B2B] transition-colors py-2 border-b border-[#E7D9D3]/40 ${
